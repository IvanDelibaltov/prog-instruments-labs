...
class ModelConfig:
    ...
    def __init__(self):
        self.Z_var: str = 'Образование'
        self.gamma: float = 0.915
        self.alpha: float = 0.085
        self.gender: int = 0
        self.experience_years: int = 12
        self.education_years: int = 10
...
class RegressionModel:
    ...
    def evaluate(self, alpha: float = 0.05) -> 'RegressionModel':
...
class RegressionAnalyzer:
    ...
    @staticmethod
    def calculate_t_test(X: np.ndarray, y: np.ndarray, model: LinearRegression, 
                        alpha: float = 0.05) -> Tuple[np.ndarray, np.ndarray, np.ndarray, 
                                                     np.ndarray, np.ndarray, np.ndarray]:
        """Выполняет t-тест для коэффициентов модели"""
        n = len(y)
        p = X.shape[1]
    ...
    @staticmethod
    def calculate_prediction_interval(X: np.ndarray, y: np.ndarray, model: LinearRegression, 
                                     x_new: np.ndarray, alpha: float = 0.05) -> Tuple[float, float, 
                                                                                      float, float, float]:
        """Рассчитывает доверительные интервалы для прогноза"""
        n = len(y)
        p = X.shape[1]
    ...
    @staticmethod
    def calculate_f_test(y: np.ndarray, y_pred: np.ndarray, k: int) -> Tuple[float, float]:
...
class ReportGenerator:
    ...
    @staticmethod
    def print_model_summary(model: RegressionModel) -> None:
        """Печатает уравнение модели"""
        print(model.get_summary())
    
    @staticmethod
    def print_f_test_results(model: RegressionModel, alpha: float) -> None:
    ...
    @staticmethod
    def print_coefficient_analysis(feature_name: str, coefficient: float, 
                                  p_value: float, alpha: float) -> None:
    ...
    @staticmethod
    def print_confidence_intervals(names: List[str], coefficients: np.ndarray, 
                                  ci_lower: np.ndarray, ci_upper: np.ndarray) -> None:
    ...
    @staticmethod
    def print_prediction_results(point_pred: float, ci_mean: Tuple[float, float], 
                                ci_individual: Tuple[float, float], experience: int, 
                                education: int, gender: int) -> None:
...
def run_analysis(config: ModelConfig) -> None:
    ...
    coeffs, se, t_stats, p_values, ci_low, ci_up = RegressionAnalyzer.calculate_t_test(
        X2.values, y.values, model2.model, config.alpha
    )
    ...
    point, ci_mean_low, ci_mean_up, ci_ind_low, ci_ind_up = RegressionAnalyzer.calculate_prediction_interval(
        X2.values, y.values, model2.model, x_new, config.alpha
    )
