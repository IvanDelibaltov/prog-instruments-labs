...
model1, X1, y = run_simple_regression(data, config.Z_var, config.alpha)
...
if f_pvalue_1 < config.alpha:
    print(f"ВЫВОД: Модель статистически значима (p = {f_pvalue_1:.4e} < α = {config.alpha}).")
    print("Она обладает умеренной объясняющей способностью.")
else:
    print(f"ВЫВОД: Модель статистически НЕ значима (p = {f_pvalue_1:.4e} > α = {config.alpha}).")
...
if f_pvalue_2 < config.alpha:
    print(f"ВЫВОД: Модель статистически значима в целом (p = {f_pvalue_2:.4e} < α = {config.alpha}).")
    print("Она обладает умеренной объясняющей способностью.")
else:
    print(f"ВЫВОД: Модель статистически НЕ значима (p = {f_pvalue_2:.4e} > α = {config.alpha}).")
    print("Она считается низкокачественной.")
...
coeffs, se, t_stats, p_values, ci_low, ci_up = t_test_sklearn(X2, y, model2, config.alpha)
...
point, ci_mean_low, ci_mean_up, ci_ind_low, ci_ind_up = prediction_interval_sklearn_fixed(X2, y, model2, x_new, config.alpha)
...
print("-" * 60)
print(f"• Множественная модель значима (p = {f_pvalue_2:.2e} < {config.alpha}) и объясняет {r2_2*100:.1f}% дисперсии.")
